...
const axiosInstance = axios.create({
    baseURL: 'http://localhost:8084/lab4_back_war/points'
});
...
const pointAPI = {
    async getPoints() {
        ...
        return axiosInstance.post('/receiving', {username}, {headers : { "Access-Control-Allow-Origin" : "*"}}).then(response => {
    ...
    async updatePoints(r) {
        let username = localStorage.getItem('user');
        return axiosInstance.post('/adding', {r, username}, {headers : { "Access-Control-Allow-Origin" : "*"}}).then(response => {
            if (response.status === 200) {
                resetPoints();
    ...
    async checkPointFromCanvas(x, y) {
        ...
        let username = localStorage.getItem('user');
        return axiosInstance.post('/adding', {x, y, r, username}, {headers : { "Access-Control-Allow-Origin" : "*"}}).then(response => {
            if (response.status === 200) {
    ...
    async clearTable() {
        let username = localStorage.getItem('user');
        return axiosInstance.delete('/removing', {headers : { "Access-Control-Allow-Origin" : "*"}}).then(response => {
            if (response.status === 200) {
                resetPoints();
            }
        })
